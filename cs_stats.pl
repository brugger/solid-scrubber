...
my ($g_reads,  $g_single, $g_double, $g_triple) = (0,0,0, 0);
my ( $g_double_good, $g_triple_good) = (0,0);

analyse( 'chrX', $start, $end);
...
#    print Dumper( $entry) if ( $id eq $name);
  ...
  print "reads: $g_reads, single: $g_single, double: $g_double, triple: $g_triple\n";
  print "double-good: $g_double_good, triple-good:  $g_triple_good avg: ".($length/$g_reads)."\n";
  exit;
